...
from modeling import CNN_DARA, WiderFirstConvCNN_DARA, DeeperSmallerCNN_DARA, KernelStrideCNN_DARA, DeeperCNN_DARA
...
def parse_arg():
    ...
    parser.add_argument('--top_k', type=int, default=1, help='top_k for task label')
...
def CV_run():
    ...
    eval_batch_size = 32
    eval_interval=10
    # label_type = "model_type" # 50 epochs, lr=1e-3
    label_type = "task"
    # label_type = "arch" # 50 epochs, lr=1e-3
    args = parse_arg()
    label_type = args.label_type
    ############################
    top_k = None
    if label_type == "task":
        vec_path = 'Naming_anomaly_detection/DARA/CNN/data/data_cleaned.json'
        multi_label = True
        top_k = args.top_k
        # vec_path = 'data_cleaned_prev.json'
    else:
        vec_path = 'Naming_anomaly_detection/DARA/CNN/data/data.json'
        multi_label = False
        # vec_path = 'data_prev.json'
    # data_loader = DataLoader(vec_path)

    full_dataset = CNN_dataset(dict_path=vec_path, label_type=label_type)
    ...
    for train_index, eval_index in kf.split(full_dataset):
        ...
        model = CNN_DARA(input_size=input_shape[1], output_size=num_classes).to(device)
    ...
    root_dir = 'Naming_anomaly_detection/DARA/CNN'
