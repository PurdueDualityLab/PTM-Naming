...
json_file_loc_params = os.path.join(PEATMOSS_PATH, "params.json")
if os.path.exists(json_file_loc_params):
    with open(json_file_loc_params, 'r') as f:
        params = json.load(f)
...
def run_all_models(repos):
    for repo in repos:
        if repo in params and params[repo] != -1:
            print(f"{repo} already exists, skipping...")
            continue
        
        start_time = time.time()
        logger.info(f"Starting evaluation for model: {repo}")
        result = subprocess.run(["python", "Naming_anomaly_detection/APTM/evaluation/latency_by_one.py", "--repo", repo])    
        end_time = time.time()
...
if __name__ == "__main__":
    set_seed(0)
    with open("Naming_anomaly_detection/data_files/json_files/selected_peatmoss_repos.json", "r") as f:
        repos = json.load(f)
